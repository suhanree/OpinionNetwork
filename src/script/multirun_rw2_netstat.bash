...
# Script for multiple runs with different random seeds. (only for netstat)

if (( $# != 16 )); then
	echo "Usage: $0 <nagents> <total_time> <Xsize> <Ysize> <g> <op_threshold> <shape> <topology> <time_interval> <number_edges> <ag_threshold> <graph_method> <neighbor_type> <movement_type> <update_method> <# of runs>"
...
then
   ...
   filename3="opinion_n"$1"_x"$3"_y"$4"_g"$5"_o"$6"_s"$7$8"_eevalue"$"_ag"${11}"_ne"${13}"_mo"${14}
   ...
   do
	...
	echo "out_network_stat_time 	$2" >> $filename_in
...
#######################################################################
   ...
   filename4="components_n"$1"_x"$3"_y"$4"_g"$5"_o"$6"_s"$7$8"_ne"${13}"_mo"${14}

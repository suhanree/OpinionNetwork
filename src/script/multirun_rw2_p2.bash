...
# Script for multiple runs with different random seeds. (specially modified to analyze transitions from period-i to period-(i-1))
#	convergence_method=4~7 & 10 (to period-0), convergence_check_period=10 (100 for convergence_method<4)

if (( $# != 17 )); then
	echo "Usage: $0 <nagents> <total_time> <Xsize> <Ysize> <g> <op_threshold> <shape> <topology> <time_interval> <number_edges> <ag_threshold> <graph_method> <neighbor_type> <movement_type> <update_method> <# of runs> <convergence_method>"
	echo "	shape: 1-rectangular, 2-torus, 3- torus with excluded points (file: excluded)"
	echo "  topology: 1-linear, 2-circular"
	echo "  time_interval < 0: no snapshot for ag, sg, agents"
	...
	echo "	movement_type: 1-von Neumann+center, 2-von Neumann, 3-Moore+center, 4-Moore [agent_max_speed=1]"
	echo "	"
	...
	echo "	summary~_nclusters: number of opinion groups for all runs (input: summary file for each run)"
...
then
   ...
   filename1="summary_n"$1"_x"$3"_y"$4"_g"$5"_o"$6"_s"$7$8"_e"$evalue"_ag"${11}"_ne"${13}"_mo"${14}
   filename2="components_n"$1"_x"$3"_y"$4"_g"$5"_o"$6"_s"$7$8"_e"$evalue"_ag"${11}"_ne"${13}"_mo"${14}
   filename3="opinion_n"$1"_x"$3"_y"$4"_g"$5"_o"$6"_s"$7$8"_e"$evalue"_ag"${11}"_ne"${13}"_mo"${14}
   filename4="rw2_n"$1"_x"$3"_y"$4"_g"$5"_o"$6"_s"$7$8"_e"$evalue"_ag"${11}"_ne"${13}"_mo"${14}
   filename5="snapshot_n"$1"_x"$3"_y"$4"_g"$5"_o"$6"_s"$7$8"_e"$evalue"_ag"${11}"_ne"${13}"_mo"${14}
   filename6="param_n"$1"_x"$3"_y"$4"_g"$5"_o"$6"_s"$7$8"_e"$evalue"_ag"${11}"_ne"${13}"_mo"${14}
   ...
   do
	...
	echo "convergence_method	${17}" >> $filename_in
	if (( ${17} < 4 ))
	then
		echo "convergence_check_period	100" >> $filename_in
	else
		echo "convergence_check_period	10" >> $filename_in
		echo "convergence_out_period	10" >> $filename_in
	fi
	...
	then
		...
		then
			...
			gawk '{if ($1=="nclusters") print $2;}' $filename_s > $filename1"_nclusters"
			gawk '{if ($1=="edges") print $2;}' $filename_s > $filename1"_edges"
			cat $filename_s > $filename1
			if (( ${17} < 1 )); then
				gawk '{if ($1=="ngroups") print $2;}' $filename_s > $filename1"_ngroups"
			else
				FindOpinions3 $6 $filename_c $filename_op $8 > $filename1"_nopinions"
				echo "time" > $filename1"_opclusters"
				cat $filename_op >> $filename1"_opclusters"
			fi
   ...
   if (( ${17} > 0 )); then
   	stat_rw2.bash $filename1"_edges" $filename1"_nclusters" $filename1"_time" > $filename4
   	stat2_rw2.bash $filename1"_nopinions" > $filename1"_nvalues"
   fi

   rm -f $executable

   gawk -v finaltime=$2 -v nrun=${16} 'BEGIN{cnt=0;}{if ($1<finaltime) cnt++;}\
	END{print cnt, nrun-cnt, nrun, cnt/nrun;}' $filename1"_time" > $filename1
   gawk -v nrun=${16} 'BEGIN{max=0;}{cnt[$1]++;if ($1>max) max=$1;}\
...
#######################################################################
   ...
   tar zcf $filename1".tgz" $filename1_each
   if (( ${17} > 4 )); then
   	filename2_each_p=$filename2"_????p"
   	tar zcf $filename2"p.tgz" $filename2_each_p
   	rm -f $filename2_each_p
   fi
   tar zcf $filename2".tgz" $filename2_each
